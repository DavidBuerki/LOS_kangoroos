...
public class VersementMapper extends Mapper {
  ...
  private final String QUERY_FIND_ALL_VERSEMENTS_BY_PARENT_ID = "SELECT A.MONTANT FROM VERSEMENTS V JOIN VERSEMENTS_ALLOCATIONS VA ON V.NUMERO=VA.FK_VERSEMENTS JOIN ALLOCATIONS_ENFANTS AE ON AE.NUMERO=VA.FK_ALLOCATIONS_ENFANTS JOIN ALLOCATIONS A ON A.NUMERO=AE.FK_ALLOCATIONS WHERE V.FK_ALLOCATAIRES=?";

  public List<VersementAllocationNaissance> findAllVersementAllocationNaissance() {
    logger.info("Executing findAllVersementAllocationNaissance");
    ...
    try {
      ...
      while (resultSet.next()) {
        ...
        versements.add(new VersementAllocationNaissance(new Montant(resultSet.getBigDecimal(2)), resultSet.getDate(1).toLocalDate()));
      }
      return versements;
    } catch (SQLException e) {
      logger.error("SQLException in findAllVersementAllocationNaissance", e);
      throw new RuntimeException(e);
    }
  }

  public List<VersementAllocation> findAllVersementAllocation() {
    logger.info("Executing findAllVersementAllocation");
    Connection connection = activeJDBCConnection();
    try {
      ...
      while (resultSet.next()) {
        logger.trace("Processing resultSet");
        versements.add(new VersementAllocation(new Montant(resultSet.getBigDecimal(2)), resultSet.getDate(1).toLocalDate()));
      }
      return versements;
    } catch (SQLException e) {
      logger.error("SQLException in findAllVersementAllocation", e);
      throw new RuntimeException(e);
    }
  }

  public List<VersementParentEnfant> findVersementParentEnfant() {
    logger.info("Executing findVersementParentEnfant");
    Connection connection = activeJDBCConnection();
    try {
      ...
      while (resultSet.next()) {
        logger.trace("Processing resultSet");
        versements.add(new VersementParentEnfant(resultSet.getLong(1), resultSet.getLong(2), new Montant(resultSet.getBigDecimal(3))));
      }
      return versements;
    } catch (SQLException e) {
      logger.error("SQLException in findVersementParentEnfant", e);
      throw new RuntimeException(e);
    }
  }

  public List<VersementParentParMois> findVersementParentEnfantParMois() {
    logger.info("Executing findVersementParentEnfantParMois");
    Connection connection = activeJDBCConnection();
    try {
      ...
      while (resultSet.next()) {
        logger.trace("Processing resultSet");
        versements.add(new VersementParentParMois(resultSet.getLong(1), new Montant(resultSet.getBigDecimal(2)), resultSet.getDate(3).toLocalDate(), resultSet.getDate(4).toLocalDate()));
      }
      return versements;
    } catch (SQLException e) {
      logger.error("SQLException in findVersementParentEnfantParMois", e);
      throw new RuntimeException(e);
    }
  }

  public boolean findVersementByParentId(long parentId) {
    logger.info("Executing findVersementByParentId with parentId: {}", parentId);
    Connection connection = activeJDBCConnection();
    try {
      PreparedStatement preparedStatement = connection.prepareStatement(QUERY_FIND_ALL_VERSEMENTS_BY_PARENT_ID);
      preparedStatement.setLong(1, parentId);
      ResultSet resultSet = preparedStatement.executeQuery();
      if (resultSet.next()) {
        logger.trace("Versement found for parentId: {}", parentId);
        return resultSet.getBoolean(1);
      }
      logger.debug("No versement found for parentId: {}", parentId);
      return false;
    } catch (SQLException e) {
      logger.error("SQLException in findVersementByParentId", e);
      throw new RuntimeException(e);
    }
  }
}
